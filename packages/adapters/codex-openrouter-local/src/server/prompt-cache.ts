...
export interface CodexOpenRouterPromptBundle {
  bundleKey: string;
  rootDir: string;
...
function resolveManagedCodexOpenRouterPromptCacheRoot(
...
async function buildCodexOpenRouterPromptBundleKey(input: {
  skills: SkillEntry[];
...
export async function prepareCodexOpenRouterPromptBundle(input: {
  companyId: string;
  skills: SkillEntry[];
  instructionsContents: string | null;
  onLog: AdapterExecutionContext["onLog"];
}): Promise<CodexOpenRouterPromptBundle> {
  const { companyId, skills, instructionsContents, onLog } = input;
  const bundleKey = await buildCodexOpenRouterPromptBundleKey({
    skills,
    instructionsContents,
  });
  const rootDir = path.join(resolveManagedCodexOpenRouterPromptCacheRoot(process.env, companyId), bundleKey);
  const skillsHome = path.join(rootDir, "skills");
  ...
  for (const entry of skills) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[paperclip] Failed to materialize Codex OpenRouter skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
