...
async function prepareCodexHelloProbe(input: {
  ...
  if (input.targetIsRemote && !input.probeApiKey) {
    const managedHome = await prepareManagedCodexOpenRouterHome(process.env, async () => {}, input.companyId, {
      apiKey: null,
  ...
  if (input.probeApiKey) {
    ...
    return {
      ...
      env: {
        ...
        OPENROUTER_API_KEY: input.probeApiKey,
      },
      cleanup: async () => {
        await cleanup();
...
export async function testEnvironment(
  ...
  if (isNonEmpty(configOpenRouterKey) || isNonEmpty(resolvedOpenRouterKey)) {
    ...
    checks.push({
      code: "codex_openrouter_api_key_present",
      level: "info",
      message: "OPENROUTER_API_KEY is set for OpenRouter-backed Codex authentication.",
      detail: `Detected in ${source}.`,
    });
  } else {
    checks.push({
      code: "codex_openrouter_api_key_missing",
      level: "error",
      message: "OPENROUTER_API_KEY is not set. Codex OpenRouter runs require an OpenRouter API key.",
      hint: "Set OPENROUTER_API_KEY in adapter env or server environment. OPENAI_API_KEY is not used by this adapter.",
    });
  }
  ...
  if (canRunProbe) {
    if (!commandLooksLike(command, "codex")) {
      ...
      const probeApiKey = resolvedOpenRouterKey;
      ...
      try {
        ...
        if (probe.timedOut) {
          ...
          checks.push({
            ...
            ...(detail ? { detail } : {}),
            hint: probeApiKey
              ? "OPENROUTER_API_KEY was provided but Codex still rejected the request. Verify the key at https://openrouter.ai/keys and that the selected OpenRouter model slug is available."
              : "Set OPENROUTER_API_KEY in this adapter's config or server environment so Paperclip can write `$CODEX_HOME/auth.json` for OpenRouter routing.",
          });
        } else {
          checks.push({
