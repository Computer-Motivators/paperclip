...
import { SANDBOX_INSTALL_COMMAND, models as codexOpenRouterFallbackModels } from "../index.js";
...
export async function ensureCodexSkillsInjected(
  ...
  const skillsHome =
    options.skillsHome ?? resolveCodexSkillsDir(resolveManagedCodexOpenRouterHomeDir(process.env));
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const configuredOpenRouterApiKey = resolveOpenRouterApiKey(envConfig, process.env);
  ...
  const desiredSkillNameSet = new Set(desiredSkillNames);
  const promptBundle = await prepareCodexOpenRouterPromptBundle({
    companyId: agent.companyId,
    skills: codexSkillEntries.filter((entry) => desiredSkillNameSet.has(entry.key)),
  ...
  if (runtimeSessionId && runtimePromptBundleKey.length > 0 && runtimePromptBundleKey !== promptBundle.bundleKey) {
    await onLog(
      "stdout",
      `[paperclip] Codex session "${runtimeSessionId}" was saved for prompt bundle "${runtimePromptBundleKey}" and will not be resumed with "${promptBundle.bundleKey}".\n`,
    );
  }
  const instructionsPrefix = combinedInstructionsContents ?? "";
  ...
  const commandNotes = (() => {
    const notes: string[] = [
      `Using stable Codex OpenRouter prompt bundle ${promptBundle.bundleKey}.`,
      repoAgentsNote,
    ];
    if (!instructionsFilePath) {
  ...
  const openRouterApiKey = resolveOpenRouterApiKey(runtimeEnv);
  ...
  const visionRun = await prepareAdapterVisionRun({
    ...
    apiUrl: runtimeEnv.PAPERCLIP_API_URL,
    apiKey: authToken ?? runtimeEnv.PAPERCLIP_API_KEY,
    openRouterApiKey,
    provider: "openrouter",
    fallbackModels: codexOpenRouterFallbackModels,
  ...
  try {
    ...
    if (
      ...
      return toResult(finalAttempt, true, true, await resolveAttemptCostUsd(finalAttempt.parsed));
    }
    ...
    return toResult(finalAttempt, false, false, await resolveAttemptCostUsd(finalAttempt.parsed));
  } finally {
    if (paperclipBridge) {
