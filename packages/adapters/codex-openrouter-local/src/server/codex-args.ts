...
function formatFastModeSupportedModels(): string {
  return `${CODEX_OPENROUTER_LOCAL_FAST_MODE_SUPPORTED_MODELS.join(", ")} or manually configured model IDs`;
}
...
export function buildCodexExecArgs(
  ...
  const fastModeApplied = fastModeRequested && isCodexOpenRouterLocalFastModeSupported(model);
